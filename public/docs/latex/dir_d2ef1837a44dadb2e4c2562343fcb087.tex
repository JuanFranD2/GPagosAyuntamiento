\doxysection{C\+:/xampp/htdocs/\+GPagos\+Ayuntamiento/node\+\_\+modules/minipass Directory Reference}
\hypertarget{dir_d2ef1837a44dadb2e4c2562343fcb087}{}\label{dir_d2ef1837a44dadb2e4c2562343fcb087}\index{C:/xampp/htdocs/GPagosAyuntamiento/node\_modules/minipass Directory Reference@{C:/xampp/htdocs/GPagosAyuntamiento/node\_modules/minipass Directory Reference}}


\doxysubsection{Detailed Description}
\hypertarget{README.md_autotoc_md5184}{}\doxysubsection{\texorpdfstring{minipass}{minipass}}\label{README.md_autotoc_md5184}
...
{\ttfamily object\+Mode} can only be set at instantiation. Attempting to write something other than a String or Buffer without having set {\ttfamily object\+Mode} in the options will throw an error.
...
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5185}{}\doxysubsection{\texorpdfstring{Usage in Type\+Script}{Usage in Type\+Script}}\label{README.md_autotoc_md5185}
The {\ttfamily Minipass} class takes three type template definitions\+:


\begin{DoxyItemize}
\item {\ttfamily RType} the type being read, which defaults to {\ttfamily Buffer}. If {\ttfamily RType} is {\ttfamily string}, then the constructor {\itshape must} get an options object specifying either an {\ttfamily encoding} or {\ttfamily object\+Mode\+: true}. If it\textquotesingle{}s anything other than {\ttfamily string} or {\ttfamily Buffer}, then it {\itshape must} get an options object specifying {\ttfamily object\+Mode\+: true}.
\item {\ttfamily WType} the type being written. If {\ttfamily RType} is {\ttfamily Buffer} or {\ttfamily string}, then this defaults to {\ttfamily Contiguous\+Data} (Buffer, string, Array\+Buffer, or Array\+Buffer\+View). Otherwise, it defaults to {\ttfamily RType}.
\item {\ttfamily Events} type mapping event names to the arguments emitted with that event, which extends {\ttfamily Minipass.\+Events}.
\end{DoxyItemize}
...
\DoxyCodeLine{}
\DoxyCodeLine{//\ a\ NDJSON\ stream\ that\ emits\ 'jsonError'\ when\ it\ can't\ stringify}
\DoxyCodeLine{export\ interface\ Events\ extends\ Minipass.Events\ \{}
\DoxyCodeLine{\ \ jsonError:\ [e:\ Error]}
\DoxyCodeLine{\}}
\DoxyCodeLine{}
\DoxyCodeLine{export\ class\ NDJSONStream\ extends\ Minipass<string,\ any,\ Events>\ \{}
\DoxyCodeLine{\ \ constructor()\ \{}
\DoxyCodeLine{\ \ \ \ super(\{\ objectMode:\ true\ \})}
\DoxyCodeLine{\ \ \}}
\DoxyCodeLine{}
\DoxyCodeLine{\ \ //\ data\ is\ type\ \`{}any`\ because\ that's\ WType}
\DoxyCodeLine{\ \ write(data,\ encoding,\ cb)\ \{}
\DoxyCodeLine{\ \ \ \ try\ \{}
\DoxyCodeLine{\ \ \ \ \ \ const\ json\ =\ JSON.stringify(data)}
\DoxyCodeLine{\ \ \ \ \ \ return\ super.write(json\ +\ '\(\backslash\)n',\ encoding,\ cb)}
\DoxyCodeLine{\ \ \ \ \}\ catch\ (er)\ \{}
\DoxyCodeLine{\ \ \ \ \ \ if\ (!er\ instanceof\ Error)\ \{}
\DoxyCodeLine{\ \ \ \ \ \ \ \ er\ =\ Object.assign(new\ Error('json\ stringify\ failed'),\ \{}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ cause:\ er,}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \})}
\DoxyCodeLine{\ \ \ \ \ \ \}}
\DoxyCodeLine{\ \ \ \ \ \ //\ trying\ to\ emit\ with\ something\ OTHER\ than\ an\ error\ will}
\DoxyCodeLine{\ \ \ \ \ \ //\ fail,\ because\ we\ declared\ the\ event\ arguments\ type.}
\DoxyCodeLine{\ \ \ \ \ \ this.emit('jsonError',\ er)}
\DoxyCodeLine{\ \ \ \ \}}
\DoxyCodeLine{\ \ \}}
\DoxyCodeLine{\}}
\DoxyCodeLine{}
\DoxyCodeLine{const\ s\ =\ new\ NDJSONStream()}
\DoxyCodeLine{s.on('jsonError',\ e\ =>\ \{}
\DoxyCodeLine{\ \ //\ here,\ TS\ knows\ that\ e\ is\ an\ Error}
\DoxyCodeLine{\})}

\end{DoxyCode}


Emitting/handling events that aren\textquotesingle{}t declared in this way is fine, but the arguments will be typed as {\ttfamily unknown}.\hypertarget{README.md_autotoc_md5186}{}\doxysubsection{\texorpdfstring{Differences from Node.\+js Streams}{Differences from Node.\+js Streams}}\label{README.md_autotoc_md5186}
...
You can avoid most of these differences entirely (for a very small performance penalty) by setting {\ttfamily \{async\+: true\}} in the constructor options.\hypertarget{README.md_autotoc_md5187}{}\doxysubsubsection{\texorpdfstring{Timing}{Timing}}\label{README.md_autotoc_md5187}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5188}{}\doxysubsubsection{\texorpdfstring{Exception\+: Async Opt-\/\+In}{Exception\+: Async Opt-\/\+In}}\label{README.md_autotoc_md5188}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5189}{}\doxysubsubsection{\texorpdfstring{No High/\+Low Water Marks}{No High/\+Low Water Marks}}\label{README.md_autotoc_md5189}
...
Since nothing is ever buffered unnecessarily, there is much less copying data, and less bookkeeping about buffer capacity levels.\hypertarget{README.md_autotoc_md5190}{}\doxysubsubsection{\texorpdfstring{Hazards of Buffering (or\+: Why Minipass Is So Fast)}{Hazards of Buffering (or\+: Why Minipass Is So Fast)}}\label{README.md_autotoc_md5190}
...
As always, the faster implementation is the one that does less stuff and waits less time to do it.\hypertarget{README.md_autotoc_md5191}{}\doxysubsubsection{\texorpdfstring{Immediately emit {\ttfamily end} for empty streams (when not paused)}{Immediately emit {\ttfamily end} for empty streams (when not paused)}}\label{README.md_autotoc_md5191}
...
However, this is {\itshape usually} not a problem because\+:\hypertarget{README.md_autotoc_md5192}{}\doxysubsubsection{\texorpdfstring{Emit {\ttfamily end} When Asked}{Emit {\ttfamily end} When Asked}}\label{README.md_autotoc_md5192}
...
To prevent calling handlers multiple times who would not expect multiple ends to occur, all listeners are removed from the `\textquotesingle{}end'\`{} event whenever it is emitted.\hypertarget{README.md_autotoc_md5193}{}\doxysubsubsection{\texorpdfstring{Emit {\ttfamily error} When Asked}{Emit {\ttfamily error} When Asked}}\label{README.md_autotoc_md5193}
...
This makes it much more difficult to end up trying to interact with a broken stream, if the error handler is added after an error was previously emitted.\hypertarget{README.md_autotoc_md5194}{}\doxysubsubsection{\texorpdfstring{Impact of "{}immediate flow"{} on Tee-\/streams}{Impact of "{}immediate flow"{} on Tee-\/streams}}\label{README.md_autotoc_md5194}
...
All of the hazards in this section are avoided by setting {\ttfamily \{ async\+: true \}} in the Minipass constructor, or by setting {\ttfamily stream.\+async = true} afterwards. Note that this does add some overhead, so should only be done in cases where you are willing to lose a bit of performance in order to avoid having to refactor program logic.\hypertarget{README.md_autotoc_md5195}{}\doxysubsection{\texorpdfstring{USAGE}{USAGE}}\label{README.md_autotoc_md5195}
It\textquotesingle{}s a stream! Use it like a stream and it\textquotesingle{}ll most likely do what you want.


\begin{DoxyCode}{0}
\DoxyCodeLine{import\ \{\ Minipass\ \}\ from\ 'minipass'}
\DoxyCodeLine{const\ mp\ =\ new\ Minipass(options)\ //\ options\ is\ optional}
\DoxyCodeLine{mp.write('foo')}
\DoxyCodeLine{mp.pipe(someOtherStream)}
\DoxyCodeLine{mp.end('bar')}

\end{DoxyCode}
\hypertarget{README.md_autotoc_md5196}{}\doxysubsubsection{\texorpdfstring{OPTIONS}{OPTIONS}}\label{README.md_autotoc_md5196}
...
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5197}{}\doxysubsubsection{\texorpdfstring{API}{API}}\label{README.md_autotoc_md5197}
Implements the user-\/facing portions of Node.\+js\textquotesingle{}s {\ttfamily Readable} and {\ttfamily Writable} streams.\hypertarget{README.md_autotoc_md5198}{}\doxysubsubsection{\texorpdfstring{Methods}{Methods}}\label{README.md_autotoc_md5198}
...
\item {\ttfamily options.\+proxy\+Errors} -\/ Boolean, proxy {\ttfamily error} events from the source stream to the destination stream. Note that errors are {\itshape not} proxied after the pipeline terminates, either due to the source emitting `\textquotesingle{}end'{\ttfamily  or manually unpiping with }src.\+unpipe(dest){\ttfamily . Default }false{\ttfamily .}
\end{DoxyItemize}
\item {\ttfamily }on(ev, fn){\ttfamily , }emit(ev, fn){\ttfamily  -\/ Minipass streams are Event\+Emitters. Some events are given special treatment, however. (See below under "{}events"{}.)}
\item {\ttfamily }promise(){\ttfamily  -\/ Returns a Promise that resolves when the stream emits }end{\ttfamily , or rejects if the stream emits }error{\ttfamily .}
\item {\ttfamily }collect(){\ttfamily  -\/ Return a Promise that resolves on }end{\ttfamily  with an array containing each chunk of data that was emitted, or rejects if the stream emits }error{\ttfamily . Note that this consumes the stream data.}
\item {\ttfamily }concat(){\ttfamily  -\/ Same as }collect(){\ttfamily , but concatenates the data into a single Buffer object. Will reject the returned promise if the stream is in object\+Mode, or if it goes into object\+Mode by the end of the data.}
\item {\ttfamily }read(n){\ttfamily  -\/ Consume }n{\ttfamily  bytes of data out of the buffer. If }n{\ttfamily  is not provided, then consume all of it. If }n{\ttfamily  bytes are not available, then it returns null. \texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*}\+Note\texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*} consuming streams in this way is less efficient, and can lead to unnecessary Buffer copying.}
\item {\ttfamily }destroy(\mbox{[}er\mbox{]}){\ttfamily  -\/ Destroy the stream. If an error is provided, then an }\textquotesingle{}error\textquotesingle{}{\ttfamily  event is emitted. If the stream has a }close(){\ttfamily  method, and has not emitted a }\textquotesingle{}close\textquotesingle{}{\ttfamily  event yet, then }stream.\+close(){\ttfamily  will be called. Any Promises returned by }.promise(){\ttfamily , }.collect(){\ttfamily  or }.concat()\`{} will be rejected. After being destroyed, writing to the stream will emit an error. No more data will be emitted if the stream is destroyed, even if it was previously buffered.
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5199}{}\doxysubsubsection{\texorpdfstring{Properties}{Properties}}\label{README.md_autotoc_md5199}
...
\item {\ttfamily encoding} Read-\/only. The encoding that has been set.
...
\item {\ttfamily object\+Mode} Indicates whether the stream is in {\ttfamily object\+Mode}.
\item {\ttfamily aborted} Readonly property set when the {\ttfamily Abort\+Signal} dispatches an {\ttfamily abort} event.
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5200}{}\doxysubsubsection{\texorpdfstring{Events}{Events}}\label{README.md_autotoc_md5200}
...
\item {\ttfamily prefinish} An end-\/ish event that follows the same logic as {\ttfamily end} and is emitted in the same conditions where {\ttfamily end} is emitted. Emitted after `\textquotesingle{}end'{\ttfamily .}
\item {\ttfamily }finish{\ttfamily  An end-\/ish event that follows the same logic as }end{\ttfamily  and is emitted in the same conditions where }end{\ttfamily  is emitted. Emitted after }\textquotesingle{}prefinish\textquotesingle{}{\ttfamily .}
\item {\ttfamily }close{\ttfamily  An indication that an underlying resource has been released. Minipass does not emit this event, but will defer it until after }end{\ttfamily  has been emitted, since it throws off some stream libraries otherwise.}
\item {\ttfamily }drain{\ttfamily  Emitted when the internal buffer empties, and it is again suitable to }write(){\ttfamily  into the stream.}
\item {\ttfamily }readable{\ttfamily  Emitted when data is buffered and ready to be read by a consumer.}
\item {\ttfamily }resume{\ttfamily  Emitted when stream changes state from buffering to flowing mode. (Ie, when }resume{\ttfamily  is called, }pipe{\ttfamily  is called, or a }data\`{} event listener is added.)
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5201}{}\doxysubsubsection{\texorpdfstring{Static Methods}{Static Methods}}\label{README.md_autotoc_md5201}
...
\end{DoxyItemize}\hypertarget{README.md_autotoc_md5202}{}\doxysubsection{\texorpdfstring{EXAMPLES}{EXAMPLES}}\label{README.md_autotoc_md5202}
Here are some examples of things you can do with Minipass streams.\hypertarget{README.md_autotoc_md5203}{}\doxysubsubsection{\texorpdfstring{simple "{}are you done yet"{} promise}{simple "{}are you done yet"{} promise}}\label{README.md_autotoc_md5203}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5204}{}\doxysubsubsection{\texorpdfstring{collecting}{collecting}}\label{README.md_autotoc_md5204}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5205}{}\doxysubsubsection{\texorpdfstring{collecting into a single blob}{collecting into a single blob}}\label{README.md_autotoc_md5205}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5206}{}\doxysubsubsection{\texorpdfstring{iteration}{iteration}}\label{README.md_autotoc_md5206}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5207}{}\doxysubsubsection{\texorpdfstring{subclass that {\ttfamily console.\+log()}s everything written into it}{subclass that {\ttfamily console.\+log()}s everything written into it}}\label{README.md_autotoc_md5207}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5208}{}\doxysubsubsection{\texorpdfstring{same thing, but using an inline anonymous class}{same thing, but using an inline anonymous class}}\label{README.md_autotoc_md5208}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5209}{}\doxysubsubsection{\texorpdfstring{subclass that defers \textquotesingle{}end\textquotesingle{} for some reason}{subclass that defers \textquotesingle{}end\textquotesingle{} for some reason}}\label{README.md_autotoc_md5209}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5210}{}\doxysubsubsection{\texorpdfstring{transform that creates newline-\/delimited JSON}{transform that creates newline-\/delimited JSON}}\label{README.md_autotoc_md5210}
...
\end{DoxyCode}
\hypertarget{README.md_autotoc_md5211}{}\doxysubsubsection{\texorpdfstring{transform that parses newline-\/delimited JSON}{transform that parses newline-\/delimited JSON}}\label{README.md_autotoc_md5211}
...
\DoxyCodeLine{\ \ \ \ if\ (}
\DoxyCodeLine{\ \ \ \ \ \ typeof\ chunk\ ===\ 'string'\ \&\&}
